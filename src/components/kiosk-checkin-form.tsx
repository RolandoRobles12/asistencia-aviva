...
// Helper to validate File objects across browsers
...
const createFormSchema = (notesRequired: boolean) =>
  z.object({
    kioskId: z
      .string({ required_error: "Por favor, selecciona un kiosco." })
      .min(1, "Por favor, selecciona un kiosco."),
    checkinType: z.enum(["entrada", "comida", "salida"], {
      required_error: "Debes seleccionar un tipo de check-in.",
    }),
    photos: z
      .array(z.custom<File>(isValidFile, "Se requiere una foto."))
...
export function KioskCheckinForm({
  ...
  const formSchema = useMemo(
    () => createFormSchema(notesRequired),
    [notesRequired],
  );
    ...
    async function initDb() {
      try {
        const database = await openDB("asistencia-aviva-db", 1, {
          upgrade(db) {
            if (!db.objectStoreNames.contains("pending-checkins")) {
              db.createObjectStore("pending-checkins", {
                keyPath: "id",
                autoIncrement: true,
              });
            }
        ...
        toast({
          ...
          description:
            "No se pudo inicializar la base de datos para el modo offline.",
        });
      }
    }
    initDb();
  ...
  const processPendingCheckins = useCallback(async () => {
    ...
    try {
      ...
      for (const checkin of pending as PendingCheckin[]) {
        ...
        if (checkin.latitude)
          formData.append("latitude", checkin.latitude.toString());
        if (checkin.longitude)
          formData.append("longitude", checkin.longitude.toString());
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (closest?.closestKioskId) {
            form.setValue("kioskId", closest.closestKioskId, {
              shouldValidate: true,
            });
            toast({
              title: "Kiosco Sugerido",
              description:
                "Hemos seleccionado el kiosco más cercano a tu ubicación.",
            });
          }
        } catch (error) {
          console.error(
            "Could not get closest kiosk, likely missing API key.",
            error,
          );
          toast({
            variant: "default",
            ...
            description:
              "No se pudo sugerir un kiosco. Por favor, selecciónalo manualmente.",
          });
        } finally {
          setLoadingLocation(false);
        }
      ...
      () => {
        toast({
          ...
          description:
            "No se pudo acceder a tu ubicación. Por favor, habilita los permisos.",
        });
        setLoadingLocation(false);
      },
    );
    ...
    const getCameraPermission = async () => {
      ...
      try {
        ...
        toast({
          ...
          description:
            "Por favor, habilita los permisos de la cámara en tu navegador.",
        });
      }
    };
    getCameraPermission();
      ...
      if (context) {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            form.setValue("photos", [...currentPhotos, file], {
              shouldValidate: true,
            });
    ...
    if (!isOnline) {
      if (!db) {
        toast({
          ...
          title: "Error",
          description:
            "La base de datos no está disponible. No se puede guardar el check-in.",
        });
      ...
      try {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "No se pudo guardar el check-in para enviarlo luego.",
        });
      } finally {
        setIsSubmitting(false);
      }
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      {hasCameraPermission === false && (
                        <Alert variant="destructive">
                          ...
                          <AlertDescription>
                            No se pudo acceder a la cámara. Por favor, revisa
                            los permisos en tu navegador.
                          </AlertDescription>
                        </Alert>
                      )}
            ...
            <Button
              ...
              disabled={
                ...
                !db ||
                (isFirebaseConfigured === false && isOnline)
              }
            >
              {isSubmitting && (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
